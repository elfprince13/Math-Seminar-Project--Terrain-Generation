...
\item The multi fractal technique uses four parameters to create a generated landscape: $\alpha$, $C_{1}$ $S$, and $H$. 
\begin{itemize}
\item The first variable, $\alpha$, is used to determine the occurrence of peaks or singularities throughout the terrain.
\item The second variable, $C_{1}$, is used to contrail the sparseness, or roughness, of the terrain.
\item The third variable, $S$, is the random number (seed) generator
\item The last variable, $H$, is the parameter that controls how smooth the terrain is.
\end{itemize}
\end{itemize}
\end{frame}
...
\item This results in a field of normalized noises, $S(\alpha)$
\end{itemize}
\item The second stage is to filter the output of stage one to get a multi-scaling behavior according to the characteristic function: $K(q) = \frac{C_{1}}{\alpha - 1}\left(q^{\alpha} - q\right)$
\begin{itemize}
\item This is done by taking the grid, $S(\alpha)$ from stage one, and transforming it to the Fourier domain using a Fast Fourier Transformation.
\item Then to obtain the multi-scaling behavior, the transformed $S(\alpha)$ is weighted with a factor and then bounded. 
\end{itemize}
\end{itemize}
\end{frame}
...
\end{itemize}
\item The fourth stage is to fractal integrate the multi-fractal field to obtain the final landscape
\begin{itemize}
\item This is controlled by the H parameter, $\beta = 2H+1$
\end{itemize}
\end{itemize}
\end{frame}
